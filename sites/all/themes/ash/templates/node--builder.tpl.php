<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> <?php if(!empty($content['field_splash_image']['#items'])): ?>splash-image<?php endif; ?> node-profile clearfix"<?php print $attributes; ?>>
  ...
  <?php
    ...
    hide($content['field_price_range']);
    hide($content['field_type']);
    hide($content['field_neighborhoods']);
  ...
  <div class="member-top-info row-fluid">
      ...
      <div class="member-top-social">
	      <ul>
		      ...
		      <li class="member-top-social-like"><div class="info">Like this builder</div> <?php print render($content['fb_social_node_builder_remodeler_associate']); ?></li>
	      </ul>
      </div>
  ...
  <div class="member-gallery row-fluid">
    <div class="span12">
      <h3 class="title gallery-title"><i class="icon-camera"></i> Builder Gallery</h3>
  ...
  <div class="member-details row-fluid">
    <div class="span12">
      <h3 class="title list-title"><i class="icon-bookmark-empty"></i> Builder Details</h3>
      <?php print render($content['field_description']); ?>
      <div class="list">
        <?php if ($node->member_company_license_number): ?>
          <div class="field field-label-inline clearfix">
            <div class="field-label">Builder License:</div>
            <div class="field-items">
              <?php print $node->member_company_license_number; ?>
            </div>
          </div>
        <?php endif; ?>
        <?php
          print render($content['field_address']);
        	...
        	print render($content['field_price_range']);
        	print render($content['field_type']);
        	print render($content['field_neighborhoods']);
  ...
  <?php print render($content['field_level']['und'][0]['value']); ?>
  
  <?php if(!empty($content['field_email'])): ?>
  <div id="contact-form" class="member-email row-fluid">
    <div class="span12">
    	 <h3 class="title email-title"><i class="icon-envelope"></i> Email this Builder</h3>
